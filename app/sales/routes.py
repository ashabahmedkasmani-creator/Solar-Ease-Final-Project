...
@role_required('sales')
def dashboard():
    # Leads = requirement submissions and completed surveys that don't have a quotation yet
    ...
    return render_template('sales_dashboard.html',
        leads_surveys=leads_surveys, leads_requirements=leads_requirements,
...
def apply_discount(quotation_id):
    ...
    q.discount = discount
    q.final_amount = subtotal - discount
    db.session.commit()
    flash(f'Discount of PKR {discount:,.0f} applied to {q.quotation_number}.', 'success')
