...
def engineer_dashboard():
    ...
    return render_template('engineer_dashboard.html', unassigned=unassigned, my_surveys=my_surveys, completed=completed, engineers=engineers)

@surveys_bp.route('/engineer/claim/<int:survey_id>', methods=['POST'])
@role_required('engineer')
def claim_survey(survey_id):
    s=Survey.query.get_or_404(survey_id)
    s.engineer=session.get('user_name'); s.status='Engineer Assigned'
    db.session.commit(); flash('Survey assigned to you.','success')
...
def update_survey(survey_id):
    s=Survey.query.get_or_404(survey_id)
    s.engineer=request.form.get('engineer',s.engineer); s.status=request.form.get('status',s.status); s.report_notes=request.form.get('report_notes',s.report_notes); s.roof_area=float(request.form.get('roof_area',s.roof_area) or 0); s.recommended_kw=float(request.form.get('recommended_kw',s.recommended_kw) or 0)
    if request.form.get('roof_direction'): s.roof_direction=request.form.get('roof_direction')
    if request.form.get('shading'): s.shading=request.form.get('shading')
    db.session.commit(); flash('Survey report updated.','success')
    return redirect(url_for('surveys.engineer_dashboard') if session.get('role')=='engineer' else url_for('admin.dashboard'))
